...
describe("Pool concurrency tracking", () => {
  it("allocates a slot with flow and repo metadata", () => {
    ...
    const slot = pool.allocate("s1", "w1", "engineering", "e1", "do stuff", "wopr-changeset", "wopr-network/wopr");
  ...
  it("activeCountByFlow returns count of active slots for a flow", () => {
    const pool = new Pool(4);
    pool.allocate("s1", "w1", "engineering", "e1", "p1", "wopr-changeset", "wopr-network/wopr");
    pool.allocate("s2", "w2", "engineering", "e2", "p2", "wopr-changeset", "wopr-network/defcon");
    pool.allocate("s3", "w3", "engineering", "e3", "p3", "other-flow", "wopr-network/wopr");
  ...
  it("activeCountByRepo returns count of active slots for a flow+repo combo", () => {
    const pool = new Pool(4);
    pool.allocate("s1", "w1", "engineering", "e1", "p1", "wopr-changeset", "wopr-network/wopr");
    pool.allocate("s2", "w2", "engineering", "e2", "p2", "wopr-changeset", "wopr-network/wopr");
    pool.allocate("s3", "w3", "engineering", "e3", "p3", "wopr-changeset", "wopr-network/defcon");
  ...
  it("allocate still works with null flow/repo (backward compat)", () => {
    const pool = new Pool(4);
    const slot = pool.allocate("s1", "w1", "engineering", "e1", "p1");
    expect(slot).not.toBeNull();
